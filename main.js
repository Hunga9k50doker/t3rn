...
const { autoBridge } = require("./scripts/auto_index.js");
const { checkBalance } = require("./scripts/checkPoints.js");
...
const mainMenu = async () => {
  consoleClear();
  console.log(colors.yellow("T3RN - Tool được phát triển bởi nhóm telegram: https://t.me/airdrophuntersieutoc"));
...
const manualBridge = async () => {
  ...
  try {
    ...
    console.log(colors.blue("Checking Wallet Balances..."));
    ...
    let amountETH;
    while (true) {
      amountETH = await inquirer.prompt([
        {
          type: "input",
          name: "amount",
          message: "Please enter the amount of ETH you want to bridge:",
          validate: (input) => {
            const value = parseFloat(input);
            if (isNaN(value) || value < 0.1) {
              return colors.red("The minimum amount is 0.1 ETH.");
            }
            return true; // Nếu hợp lệ
          },
        },
      ]);
      amountETH = parseFloat(amountETH.amount);
      break; // Thoát khỏi vòng lặp khi đã có giá trị hợp lệ
    }
  ...
  const continueAnswer = await inquirer.prompt([{ type: "confirm", name: "continue", message: "Do you wish to perform another transaction?", default: false }]);
